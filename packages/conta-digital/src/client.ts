...
export const http: AxiosInstance = axios.create({
  baseURL: env.PAYZU_HUB_API_URL,
  timeout: 30_000,
  headers: {
    Authorization: `Bearer ${env.PAYZU_TOKEN}`,
    'Content-Type': 'application/json',
    'User-Agent': `payzu-mcp-conta-digital/0.1.0`,
  },
});
...
http.interceptors.response.use(undefined, async (err: AxiosError<PayzuError>) => {
  ...
  process.stderr.write(`[payzu-mcp-conta-digital] retry ${cfg.__retry}/${MAX_RETRIES} ${cfg.method} ${cfg.url} (status=${status})\n`);
